...
- The unit/integration tests under `artifacts/api-server/src/lib/og-meta.*.test.ts` mount Express directly with supertest and intentionally **do not** exercise the artifact router. The deployment routing layer is covered by the `verify-link-previews` smoke script (see Key Commands), which is **wired into `scripts/post-merge.sh` and runs automatically after every task merge**. A failure surfaces loudly in post-merge stdout/stderr (with a banner explaining the most likely fix is clicking Publish) but is **warn-only** — it does not fail the post-merge step. The reason: production routing only takes effect after the user manually clicks Publish, which is decoupled from merges, so blocking post-merge on a successful live check would gate every merge in the window between a routing change landing and the user republishing. The signal is still visible — act on it before the next batch of share links goes out.
